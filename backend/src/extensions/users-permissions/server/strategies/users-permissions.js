...
const authenticate = async (ctx) => {
  try {
    if (ctx.request.query && ctx.request.query.token) {
      const entry = await strapi.query("api::token.token").findOne({
        select: ["token"],
        where: { token: ctx.request.query.token },
      ...
      if (!entry) {
        ...
        if (entry.user && typeof entry.token === "string") {
          return {
            authenticated: true,
            credentials: entry.user,
          };
        }
      }
    }
